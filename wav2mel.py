...
src_mixed_dir = "./result_mixed"
src_clean_dir = "./result_clean"

target_mixed_dir = "./dataset/mixed"
target_clean_dir = "./dataset/clean"
    ...
    with alive_bar(length) as bar:
        for filename in filenames:
            try:
                ...
                # 如果指定了持續時間，裁剪音頻
                if duration and (len(mixed_waveform) > duration * sample_rate or len(clean_waveform) > duration * sample_rate):
                    samples = int(duration * sample_rate)
                    mixed_waveform = mixed_waveform[:samples]
                    clean_waveform = clean_waveform[:samples]
                elif duration and (len(mixed_waveform) > duration * sample_rate or len(clean_waveform) > duration * sample_rate):
                    continue
